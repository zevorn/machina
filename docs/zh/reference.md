...
- 降低对象/设备/sysbus 样板代码的宏
- 已迁移的平台设备：UART、PLIC、ACLINT、virtio-mmio，以及
  SiFive Test 和 TMP105 的代表性竖切片
...
- `machina_impl_sysbus_device!` 为直接持有 `SysBusDeviceState` 的
  类型实现对象/设备 trait
- `machina_std_mutex_sysbus_accessors!` 覆盖常见的
  `std::sync::Mutex<SysBusDeviceState>` wrapper 模式
- `machina_parking_lot_mdevice_accessors!` 覆盖常见的
  `parking_lot::Mutex<MDeviceState>` wrapper 模式
