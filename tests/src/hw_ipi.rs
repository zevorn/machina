...
#[test]
fn task38_ipi_realizes_as_sysbus_mmio_device() {
...
#[test]
fn task38_ipi_width_and_offset_accesses_match_local_iocsr_shape() {
    let ipi = LoongArchIpi::new_named("ipi0", 1);

    ipi.mmio_write_sized(0, CORE_ENABLE + 1, 1, 0x12);
    assert_eq!(ipi.mmio_read_sized(0, CORE_ENABLE, 4), 0x0000_1200);

    ipi.mmio_write_sized(0, CORE_SET + 1, 1, 0x01);
    assert_eq!(ipi.mmio_read_sized(0, CORE_STATUS, 4), 0x0000_0100);

    ipi.mmio_write_sized(0, CORE_MAILBOX0, 8, 0x1122_3344_5566_7788);
    assert_eq!(ipi.mmio_read_sized(0, CORE_MAILBOX0 + 1, 2), 0x6677);

    ipi.mmio_write_sized(0, CORE_MAILBOX0 + 5, 1, 0xaa);
    assert_eq!(
        ipi.mmio_read_sized(0, CORE_MAILBOX0, 8),
        0x1122_aa44_5566_7788
    );
}
